from mp2_release.scripts.state import (
    Token,
    ParseState,
    shift,
    left_arc,
...
def get_deps(words_lists, actions, cwindow):
    ...
    for w_ix, words_list in enumerate(words_lists):
        ...
        parser_buff.extend(
            [Token(idx=ix + i + 1, word="[NULL]", pos="NULL") for i in range(cwindow)]
        )
        ...
        right_arc(
            state, "root"
        )  # Add te root dependency for the remaining element on stack
...
def compute_metrics(words_lists, gold_actions, pred_actions, cwindow=2):
    ...
    gold_deps = get_deps(
        words_lists, gold_actions, cwindow
    )  # Dep according to gold actions
    pred_deps = get_deps(
        words_lists, pred_actions, cwindow
    )  # Dep according to predicted actions
